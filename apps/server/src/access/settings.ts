...
export async function settingsRoute(
  ...
  const consent = await env.DB.prepare('SELECT id FROM access_devices WHERE id=? AND member_id=? AND settings_sync=1')
    .bind(deviceId, memberId).first();
  if (!consent) throw new AccessError(403, 'settings_sync_disabled');
  if (method === 'PUT') {
    const { revision, settings } = z.strictObject({
    ...
    ).bind(memberId, JSON.stringify(settings), Date.now(), revision, deviceId, memberId).first<{ revision: number; settings: string }>();
    if (!changed) {
      ...
      ).bind(JSON.stringify(settings), Date.now(), memberId, revision, deviceId, memberId).first<{ revision: number; settings: string }>();
  ...
  ).bind(memberId).first<{ revision: number; settings: string }>();
  return row
