...
import websiteChangelog from '../../website/src/changelog.json';
import {
  type Broadcast,
  broadcastEmail,
  broadcastSchema,
...
it('reads the website feed through its service binding', async () => {
  expect(await changelogEntries(bindings)).toEqual(websiteChangelog);
});

it('reports unreadable feeds and network failures as unavailable', async () => {
  const invalidJson = (async () => new Response('<html>Unavailable</html>')) as typeof fetch;
  const networkFailure = (async () => {
    throw new TypeError('Network failure');
  }) as typeof fetch;
  for (const request of [invalidJson, networkFailure, feed(published, 302)]) {
    await expect(changelogEntries(bindings, request)).rejects.toThrow('changelog_unavailable');
  }
});
