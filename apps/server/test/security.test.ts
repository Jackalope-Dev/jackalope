...
const bindings: Env = {
  ...
  ACCESS_SECRET: 'synthetic-consent-secret-'.repeat(3),
  SEQUENZY_API_KEY: 'synthetic-key',
  ACCESS_AUDIENCE_LIST: 'synthetic-list',
};
beforeAll(async () => {
...
async function latest(email: string) {
  const row = await env.DB.prepare(
    "SELECT payload FROM access_mail WHERE email=? AND kind='waitlist' ORDER BY created_at DESC,rowid DESC LIMIT 1",
  )
    .bind(email)
...
it('binds new consent to its confirmation link and never borrows prior email verification', async () => {
  ...
  await env.DB.prepare('UPDATE access_members SET waitlist_verified_at=? WHERE email=?')
    .bind(Date.now(), email)
    .run();
  await env.DB.prepare('UPDATE access_mail SET created_at=created_at-61000 WHERE email=?')
    .bind(email)
    .run();
  ...
  expect(
    await env.DB.prepare(
      'SELECT newsletter,newsletter_confirmed_at FROM access_members WHERE email=?',
    )
      .bind(email)
      .first(),
  ).toEqual({ newsletter: 0, newsletter_confirmed_at: null });
  const send = vi.fn();
  ...
  expect(
    await env.DB.prepare(
      ...
      .bind(email)
      .first(),
  ).toEqual({ newsletter: 1, newsletter_confirmed_at: expect.any(Number) });
...
it('does not confirm consent using expired tokens or revoked membership', async () => {
  for (const kind of ['expired', 'revoked']) {
    ...
    if (kind === 'expired')
      await env.DB.prepare(
        'UPDATE access_waitlist_tokens SET expires_at=0 WHERE member_id=(SELECT id FROM access_members WHERE email=?)',
      )
        .bind(email)
        .run();
    else
    ...
    expect(
      await env.DB.prepare('SELECT newsletter_confirmed_at FROM access_members WHERE email=?')
        .bind(email)
        .first(),
    ).toEqual({ newsletter_confirmed_at: null });
    await env.DB.prepare('DELETE FROM access_members WHERE email=?').bind(email).run();
  }
});
it('removes legacy unconfirmed audience membership without revoking account access', async () => {
  ...
  await register(bindings, email, true, 'inline');
  await env.DB.prepare(
    "UPDATE access_members SET status='approved',verified_at=?,sequenzy_state='{}' WHERE email=?",
  )
    .bind(Date.now(), email)
    .run();
  ...
  expect(
    await env.DB.prepare('SELECT status,newsletter_confirmed_at FROM access_members WHERE email=?')
      .bind(email)
      .first(),
  ).toEqual({ status: 'approved', newsletter_confirmed_at: null });
});
