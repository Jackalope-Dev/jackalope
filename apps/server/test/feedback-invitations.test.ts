...
it('requires confirmed newsletter consent when enrolling, queueing and delivering feedback email', async () => {
  const owner = await member();
  await env.DB.prepare('UPDATE access_members SET newsletter_confirmed_at=NULL WHERE id=?')
    .bind(owner.id)
    .run();
  await qualify(owner.id);
  ...
  expect(await env.DB.prepare('SELECT * FROM access_mail').first()).toBeNull();
  await env.DB.prepare('UPDATE access_members SET newsletter_confirmed_at=? WHERE id=?')
    .bind(start, owner.id)
    .run();
  await qualify(owner.id);
  await queueFeedbackMail(bindings, start + 3 * day);
