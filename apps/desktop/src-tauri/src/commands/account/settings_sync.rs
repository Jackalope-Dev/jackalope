...
impl SyncedSettings {
    fn validate(&self) -> Result<(), String> {
        if self.version != 1
            ...
            || !self.accent_hex.as_bytes()[1..].iter().all(u8::is_ascii_hexdigit)
            ...
            || !matches!(self.notifications.as_str(), "all" | "failures-only" | "none")
        {
...
pub enum Action {
    ...
    Read { owner: String },
    Delete { owner: String },
    Write { owner: String, revision: u64, settings: SyncedSettings },
}
...
pub async fn app_settings_sync(
    ...
    let mut view = SyncView {
        available: false, enabled: false, owner: None,
        revision: 0, settings: None, conflict: false,
    };
    ...
    bound(&record, &api)?;
    let owner = format!("{:x}", Sha256::digest(format!("{}:{}", record.origin, record.secret).as_bytes()));
    view.available = true;
    view.enabled = record.settings_sync;
    view.owner = Some(owner.clone());
    match &action {
        Action::Status => return Ok(view),
        Action::Configure { enabled } => {
            if *enabled {
                let (code, _) = request(&api, "/v1/desktop/settings/consent", reqwest::Method::POST,
                    Some(&record.secret), Some(serde_json::json!({"enabled": true}))).await?;
                if code != 200 { return Err("Could not enable settings sync. Check your account connection and retry.".into()); }
            }
            record.settings_sync = *enabled;
            state.save(&record)?;
            view.enabled = *enabled;
            if !enabled {
                let _ = request(&api, "/v1/desktop/settings/consent", reqwest::Method::POST,
                    Some(&record.secret), Some(serde_json::json!({"enabled": false}))).await;
            }
            return Ok(view);
        }
        Action::Read { owner: expected } | Action::Delete { owner: expected } | Action::Write { owner: expected, .. }
    ...
    let (method, body) = match action {
        ...
        Action::Write { revision, settings, .. } => {
            settings.validate()?;
            if revision >= 9_007_199_254_740_991 {
                return Err("Invalid settings revision.".into());
            }
            (reqwest::Method::PUT, Some(serde_json::json!({ "revision": revision, "settings": settings })))
        }
        _ => (reqwest::Method::GET, None),
    };
    let (code, data) = request(&api, "/v1/desktop/settings", method, Some(&record.secret), body).await?;
    if code == 401 {
        state.access.revoke();
        record.settings_sync = false;
        state.save(&record)?;
    ...
    if code != 200 {
        return Err(if code == 404 { "Settings sync is not available on this service yet.".into() } else { failure(code) });
    }
    view.revision = data["revision"].as_u64().filter(|v| *v < 9_007_199_254_740_991)
...
mod tests {
    ...
    fn sync_payload_rejects_secrets_unknown_fields_and_invalid_values() {
        ...
        assert!(serde_json::from_value::<SyncedSettings>(valid.clone()).unwrap().validate().is_ok());
        let mut secret = valid.clone();
        secret["apiKey"] = "private".into();
        assert!(serde_json::from_value::<SyncedSettings>(secret).is_err());
        for (key, value) in [("accentHex", serde_json::json!("bad")), ("version", serde_json::json!(2)),
            ("atmosphere", serde_json::json!(65)), ("notifications", serde_json::json!("anything"))] {
            let mut bad = valid.clone();
            bad[key] = value;
            assert!(serde_json::from_value::<SyncedSettings>(bad).unwrap().validate().is_err());
        }
    }
}
