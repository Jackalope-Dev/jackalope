...
fn create_project(parent: &Path, name: &str) -> Result<ProjectInfo, String> {
    ...
    let parent = parent.canonicalize().map_err(|_| "Choose an existing parent folder.".to_string())?;
    git(&parent, &["--version"])?;
    let path = parent.join(name);
    std::fs::create_dir(&path).map_err(|error| {
        if error.kind() == std::io::ErrorKind::AlreadyExists {
            "That folder already exists. Choose another name or open it as an existing project.".to_string()
        } else {
    ...
    initialize().map_err(|error| {
        format!("Could not initialize Git: {error} The new folder was kept at {}.", path.display())
    })?;
    Ok(ProjectInfo {
...
pub async fn task_create_project(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let parent = match parent_path {
            ...
            None => {
                std::fs::create_dir_all(&default).map_err(|_| {
...
mod tests {
    ...
    fn new_project_has_a_clean_main_branch_and_supports_the_first_worktree() {
        let parent = std::env::temp_dir().join(format!("jackalope-project-{}", uuid::Uuid::new_v4()));
        ...
        git(path, &["worktree", "add", "-b", "first-task", worktree.to_str().unwrap()]).unwrap();
    ...
    fn creation_preserves_existing_folders_and_rejects_path_escape() {
        ...
        assert!(create_project(&parent, "existing").unwrap_err().contains("already exists"));
        assert_eq!(std::fs::read_to_string(existing.join("keep.txt")).unwrap(), "keep");
        assert!(!existing.join(".git").exists());
        for name in ["", ".", "..", "../outside", "a/b", "a\\b", "C:outside", "NUL.txt", "LPT1", "trailing."] {
