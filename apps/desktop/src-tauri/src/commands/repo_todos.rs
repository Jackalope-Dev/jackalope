...
pub const TODO_FILES: [&str; 6] = [
    "TODO.md", "TASKS.md", "ROADMAP.md", "docs/TODO.md", "docs/TASKS.md", "docs/ROADMAP.md",
];
...
fn document_path(root: &Path, relative: &str) -> Result<PathBuf, String> {
    if !TODO_FILES.contains(&relative) { return Err("Unsupported TODO file.".into()); }
    ...
    for component in relative.split('/') {
        ...
        match std::fs::symlink_metadata(&path) {
            Ok(metadata) => {
                if metadata.file_type().is_symlink() { return Err("Linked TODO files or folders cannot be edited here.".into()); }
                let resolved = std::fs::canonicalize(&path).map_err(|e| e.to_string())?;
                if !resolved.starts_with(&root) { return Err("TODO file resolves outside the project.".into()); }
            }
...
fn read_document(path: &Path) -> Result<Option<String>, String> {
    ...
    if !file.metadata().map_err(|e| e.to_string())?.is_file() { return Err("The TODO path is not a regular file.".into()); }
    let mut content = String::new();
    file.take((LIMIT + 1) as u64).read_to_string(&mut content).map_err(|e| e.to_string())?;
    if content.len() > LIMIT { return Err("TODO file exceeds 256 KiB. Edit it in your code editor.".into()); }
    Ok(Some(content))
}
...
pub async fn repo_todos_read(project_path: String) -> Result<Vec<TodoDocument>, String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        if !root.is_dir() { return Err("Choose a repository folder.".into()); }
        Ok(TODO_FILES.iter().map(|relative| {
            let result = document_path(&root, relative).and_then(|path| read_document(&path));
            match result {
                Ok(content) => TodoDocument { path: relative.to_string(), content, error: None },
                Err(error) => TodoDocument { path: relative.to_string(), content: None, error: Some(error) },
            }
        }).collect())
    }).await.map_err(|e| e.to_string())?
}

fn save_document(root: &Path, relative: &str, expected: Option<&str>, content: &str) -> Result<(), String> {
    let _guard = SAVE_LOCK.lock().map_err(|e| e.to_string())?;
    if content.len() > LIMIT { return Err("TODO file exceeds 256 KiB.".into()); }
    ...
    if expected.is_none() {
        // Creation is exclusive, so a newly appeared file is never replaced.
        use std::io::Write;
        ...
        if std::fs::metadata(&path).map_err(|e| e.to_string())?.permissions().readonly() { return Err("This TODO file is read-only.".into()); }
        super::history::write_atomic(&path, content.as_bytes())?;
    }
    Ok(())
}

#[tauri::command]
pub async fn repo_todos_save(project_path: String, relative_path: String, expected_content: Option<String>, content: String) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || save_document(Path::new(&project_path), &relative_path, expected_content.as_deref(), &content)).await.map_err(|e| e.to_string())?
}
...
mod tests {
    ...
    fn saves_creates_and_rejects_stale_edits_without_touching_files() {
        ...
        assert_eq!(read_document(&root.join("TODO.md")).unwrap().unwrap(), "# Updated\n");
        ...
        assert_eq!(read_document(&root.join("docs/ROADMAP.md")).unwrap(), Some("".into()));
    ...
    fn linked_folders_cannot_redirect_writes() {
        let root = std::env::temp_dir().join(format!("jackalope-todo-links-{}", uuid::Uuid::new_v4()));
