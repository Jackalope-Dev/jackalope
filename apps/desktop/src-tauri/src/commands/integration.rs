pub(in crate::commands) mod dependencies;
...
pub(in crate::commands) fn prepare_with_message(
    directory: &Path,
    runs: &[TaskRun],
    ids: &[String],
    message: Option<&str>,
            ...
            || selected.iter().any(|run| {
                run.id != resolver.id
                    && !resolver
                        ...
                        .sources
                        .iter()
                        .any(|s| s.run_id == run.id)
            })
        {
            return Err(
                "The target or selected sources changed. Start a fresh reconciliation.".into(),
            );
        }
        for run in &selected {
            let source = snapshot(run, directory)?;
            require_verification(run, &source.tree)?;
            if run.id == resolver.id {
                dependencies::verified(run, &source.tree)?;
                let output = command(
                    &project,
                    &[
                        "grep",
                ...
                if output.status.code() != Some(1) {
                    return Err("Resolve remaining conflict markers before integration.".into());
                }
                preview_target = source.tree.clone();
            }
            plan.sources.push(source);
        }
    }
    for run in selected.iter().filter(|_| resolution.is_none()) {
        let source = snapshot(run, directory)?;
        require_verification(run, &source.tree)?;
        let base = git(
            &project,
...
pub(in crate::commands) fn load(directory: &Path, id: &str) -> Result<IntegrationPlan, String> {
    if !valid_id(id) {
...
pub(in crate::commands) fn recover_applied(
    directory: &Path,
    runs: &[TaskRun],
    id: &str,
) -> Result<Option<IntegrationPlan>, String> {
    let plan = load(directory, id)?;
    if plan.status == "applied" {
        return Ok(Some(plan));
    }
    if plan.status == "applying"
        && plan.integration_head.as_ref().is_some_and(|head| {
            command(
    ...
    {
        return apply(directory, runs, id).map(Some);
    }
    Ok(None)
}

pub(in crate::commands) fn apply(
    directory: &Path,
    runs: &[TaskRun],
    id: &str,
) -> Result<IntegrationPlan, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        coordinator.guarded_integration(&run_ids, |audits| {
            ...
            for source in &plan.sources {
                if audits
                    .iter()
                    .any(|a| a.run_id == source.run_id && a.tree.as_ref() != Some(&source.tree))
                {
                    return Err("Files changed after the scope check. Prepare review again.".into());
                }
            }
            Ok(plan)
        })
...
pub async fn integration_apply(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let directory = runtime.integration_directory();
        {
            ...
            if let Some(mut plan) = recover_applied(&directory, &runs, &plan_id)? {
        ...
        coordinator.guarded_integration(&ids, |audits| {
            let runs = runtime.integration_runs()?;
            ...
            for source in &plan.sources {
                ...
                {
                    return Err(
                        "Files changed since the scope review. Prepare review again.".into(),
                    );
                }
            }
            if cleanup == Some(true) {
...
mod tests {
    ...
    fn reconciliation_preserves_sources_and_requires_fresh_verified_resolution() {
        ...
        let mut a = f.run(1, "shared.txt", "feature a\n");
        let mut b = f.run(2, "shared.txt", "feature b\n");
        verify(&mut a);
        verify(&mut b);
        let originals = vec![a.clone(), b.clone()];
        let ids = vec![a.id.clone(), b.id.clone()];
        let input =
            dependencies::reconciliation_input(&f.plans, &originals, &ids, "reconciliation-test")
                .unwrap();
        assert!(input.reconciles);
        assert_eq!(git(&f.project, &["rev-parse", "HEAD"]).unwrap(), f.base);
        assert_eq!(
            fs::read_to_string(Path::new(&a.workspace).join("shared.txt")).unwrap(),
            "feature a\n"
        );
        let mut resolver = f.run(3, "temporary", "");
        fs::remove_file(Path::new(&resolver.workspace).join("temporary")).unwrap();
        git(
            Path::new(&resolver.workspace),
            &["reset", "--hard", &input.base],
        )
        .unwrap();
        resolver.base_head = input.base.clone();
        resolver.dependency_snapshot = input;
        verify(&mut resolver);
        let mut combined = vec![a.clone(), b.clone(), resolver.clone()];
        let mut selected = ids.clone();
        selected.push(resolver.id.clone());
        assert!(prepare(&f.plans, &combined, &selected)
            .unwrap_err()
            .contains("conflict markers"));
        fs::write(
            Path::new(&resolver.workspace).join("shared.txt"),
            "feature a and feature b\n",
        )
        .unwrap();
        verify(&mut resolver);
        combined[2] = resolver.clone();
        let plan = prepare(&f.plans, &combined, &selected).unwrap();
        assert_eq!(plan.sources.len(), 3);
        fs::write(
            Path::new(&b.workspace).join("shared.txt"),
            "changed source\n",
        )
        .unwrap();
        assert!(apply(&f.plans, &combined, &plan.id).is_err());
        fs::write(Path::new(&b.workspace).join("shared.txt"), "feature b\n").unwrap();
        assert_eq!(
            apply(&f.plans, &combined, &plan.id).unwrap().status,
            "applied"
        );
        assert_eq!(
        ...
        assert_eq!(
            fs::read_to_string(Path::new(&b.workspace).join("shared.txt")).unwrap(),
            "feature b\n"
        );
    }
