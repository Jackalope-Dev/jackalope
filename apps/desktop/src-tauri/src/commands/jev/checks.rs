...
pub(super) fn read(path: &Path) -> Result<ConnectionUsage, String> {
    ...
    if !file.exists() { return Ok(ConnectionUsage::default()); }
...
pub(super) fn record(path: &Path, response: Option<&Value>) -> Result<(), String> {
    let _guard = SETTINGS_LOCK.lock().map_err(|_| "Decision settings are unavailable.")?;
    let mut total = read(path)?;
    let reported = response.map(usage).unwrap_or_default();
    total.calls = total.calls.saturating_add(1);
    if !reported.reported { total.unreported_calls = total.unreported_calls.saturating_add(1); }
    ...
    if let Some(cost) = reported.estimated_cost_usd {
        total.usage.estimated_cost_usd = Some(total.usage.estimated_cost_usd.unwrap_or_default() + cost);
    }
    std::fs::create_dir_all(path).map_err(|_| "Could not save Jev connection-check usage.")?;
    history::write_atomic(&path.join("connection-usage.json"), &serde_json::to_vec(&total).map_err(|_| "Could not save Jev connection-check usage.")?)
}

#[tauri::command]
pub fn routing_connection_usage(runtime: State<'_, TaskRuntime>) -> Result<ConnectionUsage, String> {
    let _guard = SETTINGS_LOCK.lock().map_err(|_| "Decision settings are unavailable.")?;
...
mod tests {
    ...
    fn connection_checks_preserve_reported_totals_and_unknown_coverage() {
        ...
        record(&path, Some(&json!({"usage":{"input_tokens":100,"output_tokens":2}}))).unwrap();
