...
impl Coordinator {
    ...
    pub(super) fn startup(
        ...
        let snapshot = json!({"assignedTaskId":task,"tasks":selected,"omittedTasks":total-selected.len(),"messages":messages,"moreMessages":more,"agreementCount":inner.ledger.agreements.iter().filter(|a| a.project_id == request.project_id).count(),"scopeCheckCount":inner.ledger.scope_audits.iter().filter(|a| a.project_id == request.project_id && a.blocked()).count()});
