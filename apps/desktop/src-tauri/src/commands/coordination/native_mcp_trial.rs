...
async fn trial() -> Result<(), Box<dyn std::error::Error>> {
        ...
        .split(',')
    {
        ...
        let prompt = format!("Verify Jackalope's selected local MCP connection. {transport} Search for jackalope_echo, then call the returned read operation with value jackalope-native-trial. Do not inspect files or use other connections. Do not bypass any denied permission. Report the actual tool response, or report the permission or connection failure.");
