...
async fn trial() -> Result<(), Box<dyn std::error::Error>> {
        ...
        .split(',')
    {
        ...
        let transport = if ["grok", "antigravity", "gemini"].contains(&agent) {
        ...
        if HARNESS_AGENTS.contains(&agent) {
        ...
        loop {
            ...
            if HARNESS_AGENTS.contains(&agent) {
                for question in run.prompts.iter().filter(|question| {
                    question.status == "pending"
                        && question.input_type == "text"
                        && question.question == "Confirm this automated fixture"
                }) {
