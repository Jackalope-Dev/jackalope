    ...
    fn owned_terminal_streams_output_and_cancel_releases_the_profile() {
        ...
        loop {
            ...
            let output: String = session
                ...
                .collect();
            if !cursor_reported && output.contains("\u{1b}[6n") {
                // ConPTY inherits the cursor; emulate the response xterm sends in the app.
                session
                    .terminal
                    .as_mut()
                    .unwrap()
                    .writer
                    .write_all(b"\x1b[1;1Recho jackalope-signin-ready\r\n")
                    .unwrap();
                cursor_reported = true;
            }
            if output.contains("jackalope-signin-ready") {
                break;
            }
            drop(session);
