...
import {WindowGrant, requireValue, sameBounds, validBounds} from './guard.js';
...
const KEYS = {Tab: Clutter.KEY_Tab, Enter: Clutter.KEY_Return, Escape: Clutter.KEY_Escape,
    Space: Clutter.KEY_space, Backspace: Clutter.KEY_BackSpace, Delete: Clutter.KEY_Delete,
    ArrowUp: Clutter.KEY_Up, ArrowDown: Clutter.KEY_Down, ArrowLeft: Clutter.KEY_Left,
    ArrowRight: Clutter.KEY_Right, Home: Clutter.KEY_Home, End: Clutter.KEY_End,
...
function processStart(pid) {
    ...
    const started = text.slice(text.lastIndexOf(')') + 2).trim().split(/\s+/)[19];
...
export default class JackalopeControl extends Extension {
    enable() {
        ...
        this._seat = global.backend.get_default_seat();
        this._keys = new Set();
        ...
        this._busy = false;
        this._enabled = true;
        this._signals.push([global.stage, global.stage.connect('captured-event', (_stage, event) => this._input(event))]);
        this._signals.push([global.display, global.display.connect('notify::focus-window', () => this._verify())]);
        this._signals.push([Main.sessionMode, Main.sessionMode.connect('updated', () => this._verify())]);
        this._signals.push([Main.overview, Main.overview.connect('showing', () => this._pause('Overview opened'))]);
    ...
    _identity(window) {
        requireValue(window && !window.minimized && !window.is_hidden() && window.showing_on_its_workspace(),
        ...
        const app = window.get_wm_class() || window.get_gtk_application_id() || window.get_sandboxed_app_id();
        requireValue(typeof app === 'string' && app.length > 0, 'Window application identity is unavailable.');
        return {handle: `${this._instance}:${window.get_stable_sequence()}`, pid, started: processStart(pid),
            class: app, title: window.get_title() || app, bounds};
    }

    _windows() {
        return global.get_window_actors().map(actor => actor.meta_window).filter(window =>
            window.get_window_type() === Meta.WindowType.NORMAL || window.get_window_type() === Meta.WindowType.DIALOG);
    }

    _selected(identity) {
        const window = this._windows().find(candidate =>
            `${this._instance}:${candidate.get_stable_sequence()}` === identity?.handle);
        const current = this._identity(window);
        requireValue(['pid', 'started', 'class'].every(key => current[key] === identity[key]), 'Selected window identity changed.');
        return [window, current];
    }

    _focused(window) {
        const focus = global.stage.get_key_focus();
        return global.display.focus_window === window && !Main.overview.visible && !Main.sessionMode.isLocked &&
            (Main.sessionMode.currentMode === 'user' || Main.sessionMode.parentMode === 'user') &&
            Main.modalCount === 0 && (!focus || focus === global.stage);
    }

    _held() {
        const [, , mask] = global.get_pointer();
        const modifiers = Clutter.ModifierType.SHIFT_MASK | Clutter.ModifierType.CONTROL_MASK |
            Clutter.ModifierType.MOD1_MASK | Clutter.ModifierType.MOD4_MASK | Clutter.ModifierType.MOD5_MASK |
        ...
        return this._keys.size > 0 || this._buttons.size > 0 || this._touches.size > 0 || (mask & modifiers) !== 0;
    }

    _input(event) {
        if (this._ownDevices.has(event.get_source_device()))
            return Clutter.EVENT_PROPAGATE;
        const type = event.type();
        if (type === Clutter.EventType.KEY_PRESS) {
            this._keys.add(event.get_key_code());
            if (event.get_key_symbol() === Clutter.KEY_Escape)
                this._cancel('Escape pressed');
        } else if (type === Clutter.EventType.KEY_RELEASE) {
            this._keys.delete(event.get_key_code());
        } else if (type === Clutter.EventType.BUTTON_PRESS) {
            this._buttons.add(event.get_button());
        } else if (type === Clutter.EventType.BUTTON_RELEASE) {
            this._buttons.delete(event.get_button());
        ...
        } else if (![Clutter.EventType.MOTION, Clutter.EventType.SCROLL, Clutter.EventType.TOUCH_UPDATE,
            Clutter.EventType.TOUCHPAD_PINCH, Clutter.EventType.TOUCHPAD_SWIPE, Clutter.EventType.TOUCHPAD_HOLD].includes(type)) {
            return Clutter.EVENT_PROPAGATE;
        }
        this._resumeTicket++;
    ...
    _verify() {
        ...
        try {
            ...
            requireValue(this._bar?.visible && this._bar.mapped && !Main.sessionMode.isLocked, 'Control bar unavailable or desktop locked');
    ...
    _render() {
        if (!this._bar || !this._session)
            return;
        const active = this._session.status === 'active';
        this._label.text = active ? 'Jackalope controls this window · Esc to cancel' : `Paused · ${this._session.reason}`;
    ...
    async _resume() {
        ...
        try {
            ...
            await delay(250);
            requireValue(this._enabled && session === this._session && ticket === this._resumeTicket, 'Input interrupted Resume');
            const [currentWindow, current] = this._selected(session.window);
            requireValue(this._focused(currentWindow) && !this._held(), 'Release held input and focus the selected window');
    ...
    async _begin(request, sender) {
        ...
        const session = new WindowGrant(current, GLib.uuid_string_random() + GLib.uuid_string_random(), sender, now());
        this._session = session;
        this._ownerWatch = Gio.bus_watch_name_on_connection(Gio.DBus.session, sender, Gio.BusNameWatcherFlags.NONE, null, () => {
        ...
        try {
            ...
            await delay(150);
            requireValue(this._enabled && this._session === session && session.status === 'paused', 'Window grant ended during setup.');
            const added = this._seat.list_devices().filter(device => !before.has(device));
            requireValue(added.length === 2 && added.every(device =>
                ['Virtual pointer device for seat', 'Virtual keyboard device for seat'].includes(device.get_device_name())),
            'This compositor cannot identify dedicated virtual input devices.');
            this._ownDevices = new Set(added);
            this._bar = new St.BoxLayout({reactive: true, can_focus: true,
                style: 'background-color: #111111; color: #ffffff; padding: 10px; spacing: 12px; border-radius: 0 0 10px 10px;'});
            this._label = new St.Label({text: '', y_align: Clutter.ActorAlign.CENTER});
            this._toggle = new St.Button({label: 'Resume', reactive: true, can_focus: true,
                style: 'padding: 10px; color: #ffffff; background-color: #333333; border-radius: 6px;'});
            const cancel = new St.Button({label: 'Cancel', reactive: true, can_focus: true,
                style: 'padding: 10px; color: #ffffff; background-color: #333333; border-radius: 6px;'});
            this._toggle.connect('clicked', () => {
                if (this._session?.status === 'active')
                    this._pause('Paused by you');
                else
                    this._resume();
            });
    ...
    _point(request, current, window) {
        requireValue(Number.isSafeInteger(request.x) && Number.isSafeInteger(request.y) &&
            request.x >= 0 && request.y >= 0 && request.x < current.bounds.width && request.y < current.bounds.height,
        ...
        const actor = window.get_compositor_private();
        requireValue(picked && (picked === actor || actor.contains(picked)), 'Another window or shell surface covers the target point.');
        return [x, y];
    }
    ...
    async _capture(request, window, current) {
        ...
        requireValue(texture && texture.get_width() === current.bounds.width && texture.get_height() === current.bounds.height,
            'Capture dimensions differ from the selected window.');
        const stream = Gio.MemoryOutputStream.new_resizable();
        try {
            await new Promise((resolve, reject) => Shell.Screenshot.composite_to_stream(texture, 0, 0,
                current.bounds.width, current.bounds.height, 1, null, 0, 0, 1, stream, (_source, result) => {
                    try { Shell.Screenshot.composite_to_stream_finish(result); resolve(); } catch (error) { reject(error); }
                }));
            this._check(request);
            stream.close(null);
    ...
    async _press(request) {
        ...
        try {
            this._check(request);
            if (modifier)
                keyboard.notify_keyval(0, modifier, Clutter.KeyState.PRESSED);
            keyboard.notify_keyval(0, key, Clutter.KeyState.PRESSED);
            keyboard.notify_keyval(0, key, Clutter.KeyState.RELEASED);
        } finally {
            if (modifier)
                keyboard.notify_keyval(0, modifier, Clutter.KeyState.RELEASED);
        }
        await delay(40);
        this._check(request);
        return {status: 'sent'};
    ...
    async _call(request, sender) {
        requireValue(Meta.is_wayland_compositor() && this._enabled, 'GNOME Wayland control is unavailable.');
        ...
        if (request.action === 'list') {
            ...
            for (const window of this._windows().reverse()) {
                try { windows.push(this._identity(window)); } catch { /* Omit unavailable windows. */ }
        ...
        if (request.action === 'begin')
            return this._begin(request, sender);
        requireValue(this._session && request.token === this._session.token, 'Window grant is unavailable.');
        if (request.action === 'state') {
            this._verify();
            this._session.heartbeat(sender, now());
            if (/^#[0-9a-fA-F]{6}$/.test(request.accent) && this._toggle)
                this._toggle.set_style(`padding: 10px; color: #ffffff; background-color: #333333; border: 2px solid ${request.accent}; border-radius: 6px;`);
        ...
        try {
            ...
            if (request.action === 'screenshot')
                return await this._capture(request, window, current);
            if (request.action === 'press')
                return await this._press(request);
            ...
            if (request.action === 'click') {
                ...
                requireValue(Number.isSafeInteger(request.wheel) && request.wheel !== 0 && Math.abs(request.wheel) <= 10, 'Invalid scroll input.');
                for (let i = 0; i < Math.abs(request.wheel); i++) {
                    this._check(request);
                    pointer.notify_discrete_scroll(0, request.wheel > 0 ? Clutter.ScrollDirection.UP : Clutter.ScrollDirection.DOWN,
    ...
    CallAsync([raw], invocation) {
        if (raw.length > 128000) {
            invocation.return_dbus_error('org.jackalope.DesktopControl1.Error', 'Desktop request is too large.');
            return;
        }
        Promise.resolve().then(() => this._call(JSON.parse(raw), invocation.get_sender())).then(result => {
            invocation.return_value(new GLib.Variant('(s)', [JSON.stringify(result)]));
        }).catch(error => {
            invocation.return_dbus_error('org.jackalope.DesktopControl1.Error', String(error.message).slice(0, 1200));
        });
    }
}
