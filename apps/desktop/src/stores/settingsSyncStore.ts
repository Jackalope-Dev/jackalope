...
let paused = false;
let retryAction: 'sync' | 'enable' | 'disable' | 'delete' = 'sync';
const snapshot = () =>
  portableSettings(useThemeStore.getState().appTheme, useSettingsStore.getState());
...
export const useSettingsSyncStore = create<SyncState>()(
  persist(
    (set, get) => ({
      available: false,
      enabled: false,
      ...
      remove: async () => {
        ...
        try {
          ...
          if (generation === epoch) {
            set({ base: null, lastSynced: null });
            paused = false;
          }
        } catch (error) {
          if (generation === epoch)
            set({
              error: `Could not delete saved settings. Retry Delete synced settings when online. ${String(error)}`,
            });
        } finally {
      ...
      refresh: async () => {
        ...
        running = (async () => {
          try {
            ...
            if (
              generation !== epoch ||
              !sameSettings(before, snapshot()) ||
              useThemeStore.getState().previewing
            )
              return;
            if (!remote.enabled) {
              set({ enabled: false, base: null, lastSynced: null, conflict: null });
              return;
            }
            ...
            if (decision === 'upload') {
              ...
              if (generation !== epoch) return;
              if (!result.enabled) {
                set({ enabled: false, base: null, lastSynced: null });
                return;
              }
              if (result.conflict) {
                set({
                  error: 'Settings changed on another device. Retry to review the latest version.',
                });
                return;
              }
      ...
      resolve: async (choice) => {
        ...
        try {
          ...
          if (generation !== epoch) return;
          if (!latest.enabled) {
            set({ enabled: false, conflict: null, base: null, lastSynced: null });
            return;
          }
          if (latest.revision !== remote.revision) {
            set({
              conflict: latest,
              error: 'Saved settings changed again. Choose which version to keep.',
            });
            return;
          }
          if (choice === 'remote') {
            if (useThemeStore.getState().previewing)
              throw new Error('Finish your theme preview before restoring settings.');
            ...
            if (result.conflict)
              throw new Error('Saved settings changed again. Retry before choosing a version.');
...
export function observeSettingsSync() {
  ...
  const unsubSettings = useSettingsStore.subscribe((value, previous) => {
    if (
      !sameSettings(
        portableSettings(useThemeStore.getState().appTheme, value),
        portableSettings(useThemeStore.getState().appTheme, previous),
      )
    )
      schedule();
  });
  const unsubTheme = useThemeStore.subscribe((value, previous) => {
    if (value.appTheme !== previous.appTheme || value.previewing !== previous.previewing)
      schedule();
  });
