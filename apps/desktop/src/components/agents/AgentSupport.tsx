...
export function AgentSupport({ adapter }: { adapter: string }) {
  ...
  if (!support) return <p className="task-muted">Choose a supported adapter to see its capabilities.</p>;
  const rows = [
    ['Project context, task awareness & messages', 'Available'],
    ['User questions, browser & validation', support.bridge === 'mcp' ? 'Built-in tools' : 'Through permitted shell/network tools'],
    ['Selected project connections', support.direct.length ? 'Direct and on-demand tools' : 'On-demand tools'],
  ...
  return (
    <section aria-label="Agent support" className="mt-4">
      ...
      <p className="task-muted">Requires an installed, compatible CLI and its permissions. Messages are read at checkpoints; they do not interrupt other agents. Reported usage depends on the model.</p>
    </section>
  );
}
