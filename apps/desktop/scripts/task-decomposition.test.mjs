...
import { generateHeuristicDecomposition, multiAgentPlanningPrompt } from '../src/lib/task-decomposition.ts';
...
test('repository planning permits one task and requires actual paths and the complete objective', () => {
  ...
  const prompt = multiAgentPlanningPrompt(goal, [{ id: 'fixture', available: true }, { id: 'missing', available: false }]);
  ...
  assert.equal(readFeaturePlan(JSON.stringify(generateHeuristicDecomposition(goal, [])), 'auto').length, 1);
});
