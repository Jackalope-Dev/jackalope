...
if (process.argv.includes('--copy-icon')) {
  copyFileSync(new URL('../src-tauri/icons/128x128.png', import.meta.url), new URL('../public/app-icon.png', import.meta.url));
  process.exit(0);
}

const mark = ['antler', 'farEar', 'nearEar', 'head'].map((key) => `<path d="${paths[key]}"/>`).join('');
...
const icon = svg.replace('<g fill=', '<rect x="25" y="-8" width="128" height="128" rx="26" fill="#1d1a18"/><g fill=');
