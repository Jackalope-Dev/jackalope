...
const runners = [{ id: 'codex', name: 'Codex', available: true }, { id: 'claude', name: 'Claude', available: true }];
...
test('explicit installed preference and sole candidate remain usable', () => {
  assert.equal(routeTaskToBestAgent({ prompt: 'x', availableRunners: runners, preferredRunner: 'claude' }).agentId, 'claude');
  assert.equal(routeTaskToBestAgent({ prompt: 'x', availableRunners: runners.slice(0, 1) }).agentId, 'codex');
  assert.equal(routeTaskToBestAgent({ prompt: 'x', availableRunners: [], preferredRunner: 'missing' }).agentId, 'auto');
});
