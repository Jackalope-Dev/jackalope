...
import { applyThemeTokens, PRESET_THEMES, hslToHex } from '../src/lib/theme-engine.ts';
...
test('button labels keep at least 4.5:1 contrast across the custom color field', () => {
  ...
  globalThis.document = { documentElement: { style: { setProperty: (key, value) => tokens.set(key, value) } } };
  for (let hue = 0; hue <= 360; hue += 15) {
    for (let light = 0; light <= 100; light += 5) {
      applyThemeTokens({ ...PRESET_THEMES[0], accentHue: hue, accentSat: 80, accentLight: light });
      const channels = hslToHex(hue, 80, light).slice(1).match(/.{2}/g).map((value) => {
        const s = parseInt(value, 16) / 255;
        return s <= 0.04045 ? s / 12.92 : ((s + 0.055) / 1.055) ** 2.4;
      });
      const luminance = channels[0] * 0.2126 + channels[1] * 0.7152 + channels[2] * 0.0722;
      const contrast = tokens.get('--color-on-accent') === '#000000' ? (luminance + 0.05) / 0.05 : 1.05 / (luminance + 0.05);
