  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        detail:
          'Task updates, history loading, and rich rendering have local performance improvements. Concurrent account checks, focused codebase maps, scoped task guidance and reusable context help avoid repeated work. Connected tools can return selected data while keeping the complete response available. Local agents can use a separately checked title model, and local helpers can reuse short-lived servers. Experiments also load optional OpenCode tools only when requested, compose read-only data operations locally, search captured text for source-attributed excerpts, and apply optional Jev relevance selection with recoverable evidence. Separate workflow, source preparation, tool and continuation experiments retain correctness, complete-attempt usage, latency and missing measurements. Repository-derived repair fixtures and randomized review sets support testing; broader savings claims still need independent validation. Installed startup, memory, accessibility, saved-data recovery, and external beta trials remain release work. Required account access, optional settings sync, and feedback flows also need installed-app and delivery checks before wider use.',
      },
    ],
  },
  {
