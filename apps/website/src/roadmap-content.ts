...
export const roadmapStages = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        detail:
          'Codex, Claude Code, Grok Build, OpenCode, Kimi Code, Antigravity, and Gemini CLI adapters are implemented, with provider-specific limits. Kimi supports routing, structured questions, task tokens, and membership quota. Kimi and OpenCode accept direct project tools. Grok models and Antigravity subscription quota are detected through their CLIs. Antigravity remains a worker. Antigravity named profiles use Gemini API keys with separate billing; its subscription login is shared. Named accounts, project preferences, saved ideas, and persistent task history keep work organized. Archive old tasks individually or in bulk, then undo or restore them while keeping results and workspaces. App-wide usage charts connect task, project, and agent spending to saved outcomes, with routing overhead and missing reports visible. Project decision preferences can use local rules, an agent, or Jev with a securely stored TypeSafe key. Jev fallback uses local rules by default, with an optional agent attempt that uses agent capacity. Optional Jev discovery ranks bounded tool descriptions, and local field selection can keep large tool results concise. A public benchmark methodology separates measured fixture results from unproven general savings. Experimental batched reads, reusable source context and shadow Jev decisions have separate evaluation controls. End-to-end cost and quality improvements remain to be demonstrated. Installed-agent acceptance is still in progress.',
      },
      {
