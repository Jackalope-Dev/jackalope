...
timeout -k 10 150 dbus-run-session -- bash -c '
  set -euo pipefail
  gnome-shell --headless --wayland --no-x11 --virtual-monitor="${JACKALOPE_WAYLAND_MONITOR:-1280x840}" --wayland-display=jackalope-wayland --sm-disable >"$HOME/shell.log" 2>&1 & shell_pid=$!
  cleanup_shell() {
