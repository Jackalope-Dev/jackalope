...
with tempfile.TemporaryDirectory(prefix="jackalope-wayland-control-") as temporary:
    ...
    try:
        ...
        wait(lambda: state()[0] == "paused", "capture test grant starts paused")
        wait(resume, "Resume before external capture")
        wait(lambda: state()[0] == "active", "capture test grant active")
        capture_bus = "org.gnome.Mutter.InputCapture"
        capture_iface = capture_bus + ".Session"

        def capture_call(path, iface, method, args=None):
            return connection.call_sync(
                capture_bus,
                path,
                iface,
        ...
        try:
            serial, zones = capture_call(
                capture_path, capture_iface, "GetZones"
            ).unpack()
            width, height, x, y = zones[0]
            capture_call(
                capture_path,
                capture_iface,
                "AddBarrier",
                GLib.Variant(
                    "(u(iiii))", (serial, (x, y, x, y + height))
                ),
            )
            capture_call(capture_path, capture_iface, "Enable")
            time.sleep(0.2)
            bus(
                remote,
                remote_iface,
                "NotifyPointerMotionRelative",
                GLib.Variant("(dd)", (-20000.0, 0.0)),
            )
            wait(
                lambda: indicator.poll() is not None,
                "external input capture cancels grant",
            )
        finally:
            capture_call(capture_path, capture_iface, "Close")
        subprocess.run(
        ...
        print(
            "PASS: native Wayland inventory, Resume, accessibility, password protection, literal typing, keys, click, scroll, selected capture, keyboard/pointer interruption, focus loss, held modifiers, epochs, Escape, heartbeat expiry, external input capture and extension readiness",
            flush=True,
        )
    finally:
