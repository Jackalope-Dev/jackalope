...
See [waitlist referrals and passes](WAITLIST-REFERRALS.md) for ranked waitlists,
separate status sessions and Instant Access Pass presentation. Signup confirmation
verifies a waitlist session and any newsletter request bound to that link; it
cannot grant membership or download access.
...
approval, or require newsletter consent. Duplicate registrations without a new
newsletter request do not resend confirmations. A new optional newsletter request
can send an existing member a confirmation link, subject to the per-address
cooldown. Confirmation mail does not
...
before attempting newsletter synchronization. The confirmation token records
whether that specific request included optional product notes. Only consuming
that unexpired, single-use token confirms newsletter consent; an earlier account
or waitlist verification does not authorize a later request. The email describes
the optional subscription before confirmation. Confirmed addresses sync through
Sequenzy's subscriber API without starting a sequence.

Migration 0016 preserves account access and signup preferences, but requires new
confirmation for legacy newsletter records without consent evidence. Previously
synced, unconfirmed entries are unsubscribed during the next audience sweep;
feedback-email eligibility uses the same confirmed-consent gate. No bulk
confirmation mail is sent. Members can request confirmation through the signup
form with the optional checkbox selected.
