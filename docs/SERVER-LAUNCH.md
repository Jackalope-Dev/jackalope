# Cloudflare service deployment and operations
...
| Storage policy | Defaults: 30-day telemetry, 90-day feedback, five-minute cleanup; publish the configured policy before client activation |
...
The checked-in configuration enables `env.staging.workers_dev` for staging access
without custom DNS. Disable it when configuring a custom domain as described below.
Production disables workers.dev access and requires an explicit route.
...
dump over a live database. Rehearse recovery on staging before relying on a backup;
...
Verify the deployed service and installed client together. Local unit tests do not
establish hosted or installed behavior.

- Confirm separate environment resources and secrets, real database IDs and private R2 buckets.
- Exercise staging migrations, HTTPS, duplicate/schema/rate-limit handling and cron.
- Test private feedback triage, monitoring, backup recovery and deletion after restore.
- Compare signed downloads with candidate hashes, upload the manifest last and check caching.
- Follow [RELEASE.md](RELEASE.md) for packaged updates, tamper rejection and clean-profile checks.
- Check production readiness against the intended Worker version and release manifest.
- Before activating reporting, test installed disclosure, opt-out, explicit Send, Access login and email delivery.
