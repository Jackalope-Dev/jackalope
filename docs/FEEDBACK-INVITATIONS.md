...
This implementation extends the existing [private feedback inbox](BETA-MONITORING.md).
It is a development milestone. Migration 0010 and the matching service, website and
desktop releases must ship together before live acceptance.
...
approved/verified membership and no cooldown or completed response. Old local history
...
Apply `apps/server/migrations/0010_feedback_invitations.sql` to staging first, then
