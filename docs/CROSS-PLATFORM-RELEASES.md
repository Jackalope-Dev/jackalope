...
| Agents and accounts | Adapter protocols, account bindings, routing and continuation are shared. Validate each supported CLI's real login, account isolation, models, quota reporting and resume. Gemini CLI execution is implemented but not validated with installed profiles on any platform. Aider and Goose execution remain unimplemented everywhere. |
...
| Desktop control | Windows retains its guarded backend. macOS, Linux X11 and GNOME 46 Wayland have native integrations, visible Pause/Resume/Cancel controls, identity checks, accessibility, selected-window capture and guarded input. Devices reports readiness and explicitly installs the bundled GNOME extension or requests macOS permissions. Isolated Xvfb and GNOME fixtures exercise input and interruption. macOS compilation and installed acceptance remain open. Other Wayland compositors and GNOME versions are unavailable; XWayland alone is insufficient. See [native control details](DESKTOP-CONTROL.md). |
...
checks under X11. Its bundled extension observes compositor input and supplies
window identity, capture and the native control bar. Test extension install/enable,
disable, sign-out, physical input, IME engines and scaling on real devices. Other
compositors and GNOME versions require their own guarded integration. Standard
RemoteDesktop/ScreenCast portals and XWayland access alone do not satisfy the guard.
...
for helper prerequisites, X11/GNOME fixtures and macOS guard checks. Hardware acceptance
